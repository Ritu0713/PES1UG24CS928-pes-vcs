...
int index_load(Index *index) {
    ...
    if (!f) {
        // File doesn't exist yet — that's fine, start with empty index
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%o %64s %lu %lu %511s",
                   &mode_val, hex, &mtime, &size, path) != 5) {
            fclose(f);
            return -1;
        }
...
// Helper for qsort — sort index entries by path
...
int index_save(const Index *index) {
    ...
    // Sort a copy of entries by path
    Index sorted = *index;
